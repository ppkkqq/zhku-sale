    ...
    <div class="myorder-main container">
      <div class="order-main">
        ...
        <div class="order-head">
          ...
          <div class="head-item" style="width: 125px">我的出价</div>
          <div class="head-item" style="width: 120px">出价时间</div>
          <div class="head-item" style="width: 120px">状态</div>
        </div>
        <div class="empty-box" v-if="isEmpty">暂无数据</div>

        <div class="order-section" v-for="(pItem, index) in myPriceList" :key="index">
          <div class="order-list">
            ...
            <div class="order-item-box">
              <span class="text">{{pItem.myPrice}}</span>
            </div>
            <div class="order-item-box">
...
<script>
  ...
  import { goodsDetail, addCart, getTourismOrderList,getOrderList,getPicture,getPriceList } from "@/const/api";
  ...
  export default {
    ...
    methods: {
      async getMyComment() {
        const rst = await this.$axios.$get(`${getPriceList}?userid=${this.user}&page=${this.currentPage}`);
        console.log(12345,rst)
        this.myPriceList = rst.list;
        this.total = rst.total;
        this.pageSize = rst.pageSize;
        if (!this.myPriceList || !this.myPriceList.length)
