...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //获取旅游订单
      async gethealthCaresOrders() {
        const rst = await this.$axios.$get(`${getTourismOrderList}?onOffType=0`);
        this.healthCaresList = rst.orderInfos ? rst.orderInfos : [];
        if (!this.healthCaresList || !this.healthCaresList.length) {
          this.healthCaresEmpty = true;
        } else {
          this.healthCaresEmpty = false;
        }
      },
      //tab页切换
      onSwitchTab(index) {
